...
bool companion_start(const std::string& url, const std::string& ticket, int volume) {
    ...
    xQueueReset(playback_queue); stop_requested = false; muted = false; output_level = 0; last_output_at = 0;
...
void companion_stop() {
    stop_requested = true; muted = true; output_level = 0; last_output_at = 0;
    std::lock_guard<std::mutex> lock(socket_mutex);
