...
The shell implements real enrollment/heartbeat calls against [the management contract](../../docs/runbooks/DEVICES.md); it has been compiled, not validated on physical hardware. Display/touch, provisioning, speaker output and recovery remain local acceptance work. The `0.1.3-ota-verify-dev` candidate also adds complete-image identity reporting after checked startup confirmation and a native signed-policy verifier, tested with synthetic host fixtures. See [native OTA progress](../../docs/runbooks/NATIVE_OTA.md). Microphone capture, on-board provider sessions, the XiaoZhi Opus adapter and remote OTA execution remain pending.
