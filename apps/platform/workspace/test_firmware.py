...
class FirmwareDeliveryTests(TestCase):
    ...
    def test_signature_required_and_verified_separately(self):
        ...
        payload=json.dumps({'profile':'waveshare-7b-stock-v1','installable':True,'compatibility':CONTRACT,
    ...
    def test_expired_or_invalid_approval_blocks_metadata_and_artifacts(self):
        ...
        for expiry in ['2000-01-01T00:00:00Z', '2099-01-01T00:00:00', 'invalid', None]:
            with self.subTest(expiry=expiry):
                policy={'profile':'waveshare-7b-stock-v1','installable':True,'compatibility':CONTRACT,
    ...
    def test_valid_signature_cannot_authorize_another_p4_board_or_contract(self):
        self.client.force_login(self.user)
        root=self.root/'release';root.mkdir()
        key=Ed25519PrivateKey.generate()
        (self.root/'public').write_text(key.public_key().public_bytes(Encoding.Raw,PublicFormat.Raw).hex())
        for contract in [None,{**CONTRACT,'profile_id':'waveshare-esp32-p4-wifi6-touch-lcd-4b'},
                         {**CONTRACT,'profile_version':True},{**CONTRACT,'profile_version':2},
                         {**CONTRACT,'layout_id':'other-layout'}, {**CONTRACT,'firmware_lineage':'other-firmware'}]:
            with self.subTest(contract=contract):
                payload=json.dumps({'profile':'waveshare-7b-stock-v1','installable':True,'compatibility':contract,
                    'expires_at':(datetime.now(timezone.utc)+timedelta(days=1)).isoformat()}).encode()
                (root/'approved-release.json').write_text(json.dumps({'payload':base64.b64encode(payload).decode(),
                    'signature':base64.b64encode(key.sign(payload)).decode()}))
                self.assertFalse(self.client.get('/devices/firmware/release/').json()['installable'])
    ...
    def test_publisher_binds_exact_app_and_refuses_reuse_or_wrong_digest(self):
        ...
        with patch.object(sys, 'path', [str(settings.REPO_DIR/'firmware/tools'), *sys.path]):
        ...
        (self.root/'review-manifest.json').write_text(json.dumps({'installation_profile':'waveshare-7b-stock-v1','repository_commit':'fixture','compatibility':CONTRACT,'firmware_version':'fixture-profile-dev',
