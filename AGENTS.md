...
Current device milestone: the owner reports the first physical Waveshare 7B successfully onboarded on 0.1.13. Authenticated Wi-Fi heartbeat and configuration acknowledgement 1/1 were observed on the server. Touch/Home navigation issues, physical restoration, audio and Wi-Fi OTA remain pending; this is not production hardware acceptance. Current validation remains limited to the 7B profile; other profiles are deferred. The signed development browser installer preserves the stock layout and requires exact local backup/device comparison. See docs/runbooks/FIRST_INSTALLATION.md, docs/runbooks/BROWSER_INSTALLATION.md and docs/runbooks/NATIVE_FIRMWARE.md. See [docs/runbooks/DEVICES.md](docs/runbooks/DEVICES.md).
