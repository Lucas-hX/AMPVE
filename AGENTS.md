...
Current device milestone: the first physical Waveshare 7B completed signed nominal Wi-Fi OTAs through `0.1.17-visual-console-dev` sequence 8. The owner confirms the display, corrected touch navigation and Wi-Fi work well on the preceding build; the new authenticated console completed a live physical-mode session with four distinct device acknowledgements for mouse, touch and keyboard navigation. The speaker test still fails; local console-stop observation, heap/outage measurements, physical restoration and interrupted-update rollback remain pending, so this is not production hardware acceptance. Current validation remains limited to the 7B profile; other profiles are deferred. See docs/runbooks/FIRST_INSTALLATION.md, docs/runbooks/VISUAL_CONSOLE.md and docs/runbooks/NATIVE_FIRMWARE.md. See [docs/runbooks/DEVICES.md](docs/runbooks/DEVICES.md).
