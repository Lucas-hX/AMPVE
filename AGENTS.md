...
Current device milestone: the first physical Waveshare 7B completed the signed nominal Wi-Fi OTA to `0.1.16-touch-ota-dev`. The owner confirms the display, corrected touch navigation and Wi-Fi work well. The speaker test still fails; physical restoration and interrupted-update rollback remain pending, so this is not production hardware acceptance. The authenticated semantic visual console and a compiled `0.1.17-visual-console-dev` client are implemented; physical remote-control behavior remains pending. Current validation remains limited to the 7B profile; other profiles are deferred. See docs/runbooks/FIRST_INSTALLATION.md, docs/runbooks/VISUAL_CONSOLE.md and docs/runbooks/NATIVE_FIRMWARE.md. See [docs/runbooks/DEVICES.md](docs/runbooks/DEVICES.md).
