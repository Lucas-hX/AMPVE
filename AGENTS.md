...
Current device milestone: the first physical Waveshare 7B completed signed nominal Wi-Fi OTAs through `0.1.17-visual-console-dev` sequence 8. The owner confirms display, corrected touch navigation, Wi-Fi and visual-console hardware behavior work excellently and accepted closing #84. The live console recorded four distinct device acknowledgements for mouse, touch and keyboard navigation. The speaker test still fails; long-run resource work, physical restoration and interrupted-update rollback remain pending, so this is not production hardware acceptance. Companion is the next application milestone. ADR 0006 defines a resident AMPVE Core plus future bounded portable packages; arbitrary native uploads do not preserve manageability. Current validation remains limited to the 7B profile; other profiles are deferred. See docs/runbooks/FIRST_INSTALLATION.md, docs/runbooks/VISUAL_CONSOLE.md and docs/runbooks/NATIVE_FIRMWARE.md. See [docs/runbooks/DEVICES.md](docs/runbooks/DEVICES.md).
