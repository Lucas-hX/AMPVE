...
with sync_playwright() as p:
    browser=p.chromium.launch();context=browser.new_context(has_touch=True,viewport={'width':1440,'height':1000})
    ...
    def route(r):
        ...
        if path==f'/devices/{device_id}/console/session/{session_id}/' and req.method=='GET':
            payload={'protocol':1,'active':True,'expires_at':'2030-01-01T00:10:00Z','device_connected':True,
                ...
                'last_command':{'id':str(uuid.uuid4()) if commands else None,'result':'applied' if commands else None}}
        ...
        if path.endswith('/commands/') and req.method=='POST':
            payload=req.post_data_json;commands.append(payload)
