...
4. Set In Progress only for actual work. Hardware required and Needs decision are explicit readiness states. Lucas reports two completed matching Windows reads and confirms stock startup after reset. The owner has now confirmed a separate-disk backup copy; those facts do not establish a physical restore. The stock-preserving/browser implementation is tracked in ADR 0004 and the browser installation runbook.
