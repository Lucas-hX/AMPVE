...
## Native candidate implementation update (2026-09-13)

PR #4 was merged after the owner's successful tests. The native LVGL overlay, HTTPS management client, local read-only two-read backup tool, pinned IDF 6.1 build and non-installable review bundle tooling are implemented; see [the native runbook](../runbooks/NATIVE_FIRMWARE.md) for verified build evidence and remaining physical gates. Provisioning now uses a per-boot WPA2 password, an AP-interface-only HTTP portal with local Host/Origin checks, escaped SSID display and a bounded setup window. No C6 firmware, eFuses, camera, microphone capture or remote OTA is enabled. Name/volume acknowledgement is withheld when an unsupported unmute is requested. This implements the first shell candidate, not hardware acceptance or a safe-to-flash release.
