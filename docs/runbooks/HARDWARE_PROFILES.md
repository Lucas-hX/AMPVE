...
The first physical 7B reported the exact schema-2 tuple, P4 revision 1.3, 32 MiB flash, 32 MiB PSRAM and Wi-Fi `passed` while running 0.1.13. The server observed authenticated heartbeat and configuration acknowledgement 1/1. This reconciles the P4 profile and current network path with the owner-local audit. It does not identify the installed C6 image: the profile continues to expose that uncertainty as a named compatibility blocker instead of treating the P4 result, MAC address or USB VID/PID as proof.
