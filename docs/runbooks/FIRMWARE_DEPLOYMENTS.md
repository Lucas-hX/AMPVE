...
Verified release-note update (2026-09-13): eligible versions now show plain-text feature notes from the review archive after its digest and the publisher policy are verified. Recovery review remains a distinct field. Missing or invalid optional notes show an explicit unavailable message, and changed archive bytes fail release eligibility. See [FIRMWARE_RELEASES.md](FIRMWARE_RELEASES.md) for authoring, bounds and signature coverage. This completes the software note-display path; physical update/rollback acceptance remains pending.
