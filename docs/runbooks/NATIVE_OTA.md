...
Candidate `0.1.9-wifi-storage-dev` integrates the native update worker with the [persistent backend](FIRMWARE_DEPLOYMENTS.md). The code implements authenticated polling, signed-policy verification, app-only inactive-slot streaming, progress, durable report retries, local cancellation and confirmed startup/rollback outcomes. Physical Wi-Fi OTA and recovery remain **unvalidated** (#26–#27). No board has been flashed by this implementation work.
...
The authorized follow-up installed `0.1.17-visual-console-dev` sequence 8 for the same first 7B. Two builds reproduced its 2,881,536-byte app, SHA-256 `ebe00e7fd5327efa57b797b4d921d17e8a8a253e214c7e60c06fdef383075cc6`; signed release `3f9eeca4be7306b37c9052292ad66792e6e30e34bf68b1974499080244823b0c` names only the confirmed sequence-7 predecessor. Deployment `63ce7b90-2618-4a60-942a-ed2eb15f2b88` completed 15 ordered reports, exact target identity and startup confirmation with no attention flag. The device then acknowledged bounded visual-console mouse, touch and keyboard navigation. Interrupted download, rollback, power loss and USB restoration remain untested.

## Earlier foundation evidence

PR #44's `0.1.3-ota-verify-dev` app was 2,768,208 bytes, SHA-256 `4af935c10210ea1dac5aa21b2e2172117a5856773fc2945857d4ac37cd7fb85e`, with 61 policy, nine streaming-identity and 26 firmware-tool cases passing. Its private non-installable review package was traced to source `0046077`. That result did not include the integrated client. That build did not inherit the earlier candidate's two-build reproduction evidence.
...
Validation passed: **61 policy cases**, **nine streaming-identity cases**, **21 client scenarios**, **31 firmware-tool tests**, **67 Django tests** (five PostgreSQL-only cases skipped there) and **21 isolated PostgreSQL lifecycle/concurrency cases**. The backend status/boot-selection-failure support was restarted on the VPS; no database migration was needed, platform/audio/database remained active and public HTTPS health passed. The two existing HSTS subdomain/preload warnings remain. Adapter calls compiled against ESP-IDF, but actual device HTTP/flash faults and physical power interruption are still acceptance work. That integration did not yet demonstrate build reproduction; the subsequent recovery revision below has separate evidence.


## Recovery evidence and reproduction — 2026-09-13

The `0.1.5-ota-recovery-dev` enabled software fixture is **2,843,920 bytes**, SHA-256 `cc4a91ccd1e2b6763fa1bc5d9db83b0dd3d5555791800d0ea0fb27d0b68e2528`, with 31% remaining in each OTA slot. The private `firmware-ota-recovery-software-fixture` package is traced to source `11986c6`, with `testing_only: true` and `installable: false`. The public installer selection is unchanged.

The application, bootloader, partition table and initial OTA data matched byte-for-byte across two separately compiled project directories on this VPS, sharing the pinned IDF/toolchain and registry sources. The second checkout was fresh. The initial comparison rejected a stale incremental client object in the first directory; explicitly recompiling that object produced the matching application above. The rejected initial hash is not release evidence. Cross-machine reproduction remains untested.

**29 client scenarios**, **61 policy cases** and **nine identity cases** passed under ASan/UBSan, plus **31 firmware-tool tests**. New scenarios cover stale failed-slot metadata, unreadable recovery state, selection-journal write failures, schema-1 migration with pending reports and authorization rejection before selection. These simulate the hardware/transport boundary. The native adapter compiled against ESP-IDF without compiler warnings; real HTTP/flash-driver faults, physical power interruption and rollback remain unvalidated. No backend migration, service restart, production signing key or hardware write was required.
