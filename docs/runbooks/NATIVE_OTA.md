...
The corrected trial uses candidate `0.1.16-touch-ota-dev` and fresh native build sequence 7. During OTA, the client selects the inactive `ota_0` slot at `0xA10000`, streams and hashes the complete app, changes boot selection only after verification, then requires confirmed startup. The retained USB backups remain the fallback if automatic rollback or network recovery fails. Exact sequence-7 build and deployment evidence is recorded after the fresh builds complete.
