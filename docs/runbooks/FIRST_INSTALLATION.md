...
The owner reports touch/Home navigation problems. The pinned XiaoZhi adapter configured the GT911 with both coordinate mirrors disabled, while [Waveshare's maintained 7B example](https://github.com/waveshareteam/ESP32-P4-WIFI6-Touch-LCD-7B/blob/main/examples/esp-idf/08_lvgl_display_panel/main/main.c) uses `swap_xy=0`, `mirror_x=1`, `mirror_y=1`. Candidate `0.1.14-touch-fix-dev` applies those flags only to the 7B profile. Nineteen host display/touch scenarios pass, and two same-host builds produced byte-identical 2,876,272-byte applications with SHA-256 `bbff470ae03055ea36c93534468e36e979b98b121b75dcb63582351bbe639a22`. This is an unsigned, non-installable review candidate; physical coordinates and navigation remain unverified.

A controlled Wi-Fi update is technically implemented, but the first attempt is not ready to queue. Import the already signed 0.1.13 initial-install release, bring the device online so it reports that exact confirmed application identity, and publish a reviewed OTA successor whose predecessor list contains 0.1.13 SHA-256 `ca04872053e00abd412ef9f074ed71f43efa39477487d74e423dd20de964d1b9`. Keep the exact USB recovery files available. Signing, importing and queueing the successor require a separate reviewed operational step; physical rollback remains pending in #27.
