...
The owner reports successful browser onboarding and the first registered device on 0.1.13. The server observed authenticated Wi-Fi heartbeat, configuration acknowledgement 1/1 and the exact confirmed sequence-4 application identity. On 2026-09-14 the first controlled Wi-Fi OTA completed and the device confirmed sequence 7, version `0.1.16-touch-ota-dev`. This validates the registration, settings and nominal OTA round trip; physical touch behavior, audio, power-interruption rollback and original restoration remain separate evidence.
...
## First registered device and touch follow-up — 2026-09-14
...
Two clean sequence-7 builds produced the same 2,876,272-byte application, SHA-256 `d21e1a5241e16b3f5bcb02270ad6a8b85f48f5df2d36a366fb4bf36caba8d3b4`, with 30% free in either application slot and USB commissioning disabled. Signed release `7e794b2e315c7cea20531e129938d0d7c7747092ba592517703b2e10cd3d0abd` was offered only to the confirmed device. Deployment `81cc1f31-a682-4ae2-b400-fdc10193b284` advanced through download, verification and reboot; the device reported 15 ordered events, all 2,876,272 bytes and confirmed the target hash at sequence 7. A later authenticated heartbeat reports `0.1.16-touch-ota-dev`, configuration acknowledgement 1/1 and Online status. This proves the nominal Wi-Fi OTA path on the first 7B; it does not yet prove touch behavior, power-loss rollback or original restoration.
