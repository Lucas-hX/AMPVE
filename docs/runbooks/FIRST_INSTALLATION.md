...
The final runtime allocation also retains IDF's `TASK_EXTRA_STACK_SIZE` allowance: the configured 16 KiB budget is 16,896 bytes in this pinned build, while the early main task requests 4,608 bytes. The reduction remains exactly 12 KiB before scheduler startup. Final binary/publication evidence follows after the independent builds complete.
