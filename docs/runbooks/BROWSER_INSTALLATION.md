...
Implementation: browser audit, local backup/import, signed-release gate, exact plan/recovery preparation, bounded app-first writer and pairing handoff. **A signed USB-assisted development release is active. The owner reports a first attempt with a black screen after power cycling; physical startup/recovery remains unconfirmed.** See [ADR 0004](../decisions/0004-stock-preserving-browser-installation.md).
...
Automatic C6/recovery update: the 7B flow now supports a curated RAM-only C6 query after board confirmation and a full read-only recovery comparison after candidate preparation when the board is connected. Both retain installation gating. See [C6_DIAGNOSTIC_RECOVERY.md](C6_DIAGNOSTIC_RECOVERY.md) for source/build checks, exact restoration intervals and the still-required physical trials.

Failed first installation: use **Recover a device that did not start**, confirm the model and import the original two backups plus audit record. Recovery no longer requires another installation or a serial connection before preparing the local plan. USB startup checks can export a sanitized failure summary, and restoration still requires explicit confirmation and fresh full-flash comparison. See FIRST_INSTALLATION.md for the owner report and limits.
