...
5. **Prepare AMPVE installation** downloads and validates the current app, compares it with the local verified backup and derives the exact proposed regions without USB writes. **Save private recovery files** writes and rehashes the originals locally. **Download installation review summary** exports only fingerprints, numeric boot selection and proposed offsets/sizes/hashes for review. No local terminal is needed. An unsigned review plan remains blocked from installation; after publication, prepare again against the signed release.
