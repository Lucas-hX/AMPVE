...
- Startup has a separate 60-second local check: UI ticks, touch initialization, local Wi-Fi driver initialization, management task and storage readiness. It does not require Wi-Fi association, internet or provider access. A successful check confirms a pending OTA app; failure attempts SDK app rollback and otherwise restarts. Three unconfirmed boots stop peripheral initialization. In this bounded recovery state, release BOOT and then hold it continuously for five seconds to retry: only the AMPVE boot counter is cleared. Boot-counter read/type/write errors stop driver startup and never cause automatic erasure. A failed physical retry requires another release and hold before any new write attempt. Essential LCD/C6 driver paths and the boot guard still require physical validation.
...
Software evidence (2026-09-13): **21 audio failure/cleanup scenarios** passed under ASan/UBSan and **31 firmware-tool tests** passed. The enabled P4 revision-1.x fixture compiled without compiler warnings and passed slot capacity checks: **2,840,544 bytes**, SHA-256 `362ff10c21782464d920888a728e83422c3b6242a04277083c5fb2de929b04ce`, 31% slot space remaining. The built codec transformation and overlay bytes matched the repository inputs. Private package `firmware-audio-guard-software-fixture` is traced to `d7f14cc`, explicitly testing-only and non-installable. This revision has one build, so it does not inherit the prior OTA revision's two-directory reproduction evidence. No public installer selection, production key, service configuration or hardware was changed.


## Boot-counter and physical retry handling

Candidate `0.1.7-boot-guard-dev` checks the boot counter before peripheral initialization. Only a genuinely absent `boots` key starts from zero. A wrong type or failed read preserves the existing state and stops startup; a failed increment or commit also stops startup. Values at or above three enter bounded recovery without incrementing, including oversized/corrupt high values. Nothing in this path erases NVS or modifies network/device credentials.

The physical retry first requires a released BOOT input, then a continuous five-second hold measured by the monotonic ESP timer. A stuck/held input at recovery entry cannot initiate a retry. Input configuration failure stops the retry path. Each completed hold permits exactly one counter-reset attempt; a failed write/commit requires release and another complete hold, preventing repeated writes while the button remains pressed. A successful committed reset restarts the app. This same checked reset function clears the counter after successful local startup diagnostics.

Run the actual boot guard with simulated NVS and button/timer inputs:

```bash
python3 firmware/tests/native/boot_run.py
```
...
```bash
python3 firmware/tests/native/wifi_startup_run.py --work "$HOME/.cache/ampve-firmware/xiaozhi-ota"
```

The Wi-Fi runner compiles the actual transformed manager initializer against simulated NVS/netif/event-loop/driver APIs. It injects failures at each stage, including both NVS errors formerly triggering erase, accepts an already-created netif/event loop and verifies that successful repeated initialization is a no-op. The guarded override is applied both to existing build directories and during first-time provisioning-component creation. The original component and its license remain preserved in the private build cache/review integration.

Software evidence (2026-09-13): **18 display startup scenarios** and **14 Wi-Fi startup scenarios** passed under ASan/UBSan; **31 firmware-tool tests** passed. The final P4 revision-1.x fixture compiled without compiler warnings and matched the repository's overlay, board/MIPI transformations and Wi-Fi manager transformation. App: **2,841,232 bytes**, SHA-256 `483bc70c3f0ddc313083d9342287727641d86aedb3b40d1cd39ccc26c552b260`, 31% remaining in each OTA slot. Private `firmware-startup-guard-software-fixture` is traced to `322b7aa`, testing-only and non-installable. This changed revision has no two-build reproduction claim. No hardware, public installer selection, production signing key or service configuration changed.
