...
export function selectBoot(data) {
  ...
  return {offset:0x10d000+index*4096,bytes,current:current||null};
}
...
export async function verifyRelease(data, now=Date.now()) {
  ...
  ensure(Number.isInteger(policy.app.size) && policy.app.size>0 && policy.app.size<=0x3f0000 && policy.app.offset===0xe00000,'App exceeds the stock target slot.');
  return policy;
}
...
export async function makePlan(file, report, policy, app) {
  ...
  const table=new Uint8Array(await file.slice(0x8000,0x9000).arrayBuffer());
  ...
  const otadata=new Uint8Array(await file.slice(0x10d000,0x10f000).arrayBuffer()),selection=selectBoot(otadata);
  ...
  return {profile:PROFILE,backup_sha256:report.sha256,app_sha256:policy.app.sha256,
    writes:[{name:'AMPVE application',offset:0xe00000,bytes:appPadded,sha256:await sha256(appPadded)},
      {name:'Boot selection (one sector)',offset:selection.offset,bytes:selection.bytes,sha256:await sha256(selection.bytes)}],
    recovery:[{name:'restore-otadata.bin',offset:0x10d000,bytes:otadata},
      {name:'restore-nvs-after-boot.bin',offset:0x3b000,bytes:new Uint8Array(await file.slice(0x3b000,0x10d000).arrayBuffer())},
      {name:'restore-ota1-touched-sectors.bin',offset:0xe00000,bytes:new Uint8Array(await file.slice(0xe00000,0xe00000+appPadded.length).arrayBuffer())}]};
}
...
export async function executePlan(reader, plan, policy, consent, progress=()=>{}, signal) {
  ...
  ensure(plan.writes.length===2 && plan.writes[0].offset===0xe00000 && plan.writes[0].bytes.length<=0x3f0000 &&
    [0x10d000,0x10e000].includes(plan.writes[1].offset) && plan.writes[1].bytes.length===4096,'Unexpected write regions.');
