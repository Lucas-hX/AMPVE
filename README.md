...
**Current status:** Django platform foundation with an English frontend, private accounts, profile/password settings, administration, and a Companion preview catalog. Encrypted provider connections and an experimental FastAPI/Pipecat browser voice preview are implemented; Lucas reports a successful OpenAI conversation; Gemini still needs account validation. Browser audit, private backup/import and a gated stock-preserving installer are implemented alongside the device-management backend. The owner reports the first physical 7B installation and registration on the signed 0.1.13 development release; authenticated Wi-Fi heartbeat and settings acknowledgement were observed on the server. A reproducible 0.1.14 development candidate corrects the 7B touch coordinate transform, but is not signed or approved for installation. Physical touch/navigation, recovery, audio and Wi-Fi OTA remain pending. See the [browser installation runbook](docs/runbooks/BROWSER_INSTALLATION.md), [native firmware runbook](docs/runbooks/NATIVE_FIRMWARE.md) and [platform runbook](docs/runbooks/PLATFORM.md).
