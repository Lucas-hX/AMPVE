...
**Current status:** Django platform foundation with an English frontend, private accounts, encrypted provider connections and an experimental browser voice preview; Lucas reports a successful OpenAI conversation, while Gemini remains unvalidated. The first physical 7B completed a signed Wi-Fi OTA to `0.1.16-touch-ota-dev`; Lucas confirms its display, corrected touch navigation and Wi-Fi work well. Speaker output, physical restoration and interrupted-update rollback remain pending. An authenticated semantic visual console and the compiled `0.1.17-visual-console-dev` device client are now implemented for owner sessions, mouse/touch/keyboard input and future headless virtual displays; physical console behavior remains unvalidated. See the [visual console](docs/runbooks/VISUAL_CONSOLE.md), [native firmware](docs/runbooks/NATIVE_FIRMWARE.md) and [platform](docs/runbooks/PLATFORM.md) runbooks.
